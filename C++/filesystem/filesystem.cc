...
int FileSystem(char diskName[16]){
	// open the file with the above name
		...
		if(fptr == NULL){ 
			...
			execl(path, path, diskName, (char*)0);//attempts to go to create_fs program and make the required disk
...
int creater(char name[8], int size){
    ...
    // allocate a new file of this size
		...
		for(i=0;i<128;i++){
			...
			if(freeBlockList[i]==0){//count goes up for each empty space, if there is room, go ahead
				count++;
			}
			if(i==63)cout<<endl<<"\t     ";
		}
    ...
    // end for
		int j;
		if(frag!='y')fbptr=0;
		for(i=0;i<size;i++){//had to add in a fragmented option because my system doesn't fragment
			for(j=fbptr;j<128;j++){//it fills in the first available spots each time.
...
//----------------------------------------------------------------------
    ...
    cin>>frag;
    if(frag=='y')//to toggle fragmented option
...
//----------------------------------------------------------------------
	...
	while( fgets(buffer, 30, input) != NULL ){// while there are still lines in the input file
		//j++;
		...
		int result = -1;
		int a;
		switch(*args[0]){//NOTE: will not create a disk that starts with these characters
			...
			case 'L':	cout<<YELLOW"Listing"RESET<<"..."<<endl;
						result = ls();
						break;
			case 'P':	cout<<RED"PRINTDISK..."RESET<<endl;//No longer works
						rewind(fptr);
						for(i=0;i<128;i++){
						...
						break;
			case 'Z':	cout<<RED"REFORMAT..."RESET<<endl;//put at the beginning/end of the file for fast erase
						for(i=0;i<128;i++){
							freeBlockList[i]=0;
						}
						freeBlockList[0]=1;
						...
						result=0;
						break;
			default://making the file for anything that does not start with switch characters
						last = strtok(args[0]," \n\?");
